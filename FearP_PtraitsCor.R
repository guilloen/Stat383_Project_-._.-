...
S2C = cor(data$FearP, data$Trust)
S2D = cor(data$FearP, data$SelfA)
S2E = cor(data$FearP, data$Affec)
...
df <- data.frame( "Correlation Table" = c("Y1 (Fearful of People)", 
                                          ...
                                          "X3 (Affectionate)"),
                  "Y1 (Fearful of People)" = c(S2B, S2C, S2D, S2E),
                  "X1 (Trusting)" = c("", S3C, S3D, S3E),
                  "X2 (Self Assured)" = c("", "", S4D, S4E),
                  "X3 (Affectionate)" = c("", "", "", S5E))

write_xlsx(df, "C:/Users/Elizabeth/Documents/Fall 2023/STAT 383/STAT Project/Some helpful codes-20231203/FearP_PtraitsCor.xlsx")